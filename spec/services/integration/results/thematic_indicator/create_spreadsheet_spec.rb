...
describe Integration::Results::ThematicIndicator::CreateSpreadsheet do

  let(:indicator) { create(:integration_results_thematic_indicator, created_at: date) }
  let!(:old_indicator) { create(:integration_results_thematic_indicator, created_at: date - 1.month) }

  let(:date) { Date.today.beginning_of_month }
  let(:month) { date.month }
  let(:year) { date.year }
  let(:year_month) { I18n.l(Date.new(year, month), format: '%Y%m') }
  let(:base_path) { '/public/files/downloads/integration/results/thematic_indicators' }

  subject(:service) { Integration::Results::ThematicIndicator::CreateSpreadsheet.new(year, month) }
  ...
  describe 'resources' do
    it 'filters thematic_indicator' do
      indicator
  ...
  context 'spreadsheet' do
    it 'adds worksheet with title' do
      expected_title = I18n.t("integration/results/thematic_indicator.spreadsheet.worksheets.default.short_title", month: month, year: year)
    ...
    it 'adds header for worksheet_type' do
      indicator
      expect(service).to receive(:xls_add_header).with(anything, Integration::Results::ThematicIndicator::SpreadsheetPresenter.spreadsheet_header)
  ...
  context 'csv' do
    it 'converts xls to csv' do
      ...
      xls_filename = Rails.root.to_s + "#{base_path}/#{year_month}/results_thematic_indicator_#{year_month}.xlsx"
      csv_filename = Rails.root.to_s + "#{base_path}/#{year_month}/results_thematic_indicator_#{year_month}.csv"
