...
describe Integration::Expenses::Npfs::CreateStats do
  ...
  describe 'data' do
    it 'total' do
      npf_data = base_npf_data

      first = create(:integration_expenses_npf, npf_data)
      second = create(:integration_expenses_npf, npf_data)

      ignored_out_of_range = create(:integration_expenses_npf, npf_data.merge({data_emissao: date - 2.years}))
      ...
      Integration::Expenses::Npfs::CreateStats.call(year, month)

      generated = Stats::Expenses::Npf.find_by(year: year, month: month)

      expect(generated.data[:total]).to eq(expected_data[:total])
    end

    it 'management_unit' do
      npf_data = base_npf_data.merge({
        management_unit: management_unit,
      ...
      undefined_management_unit = build(:integration_expenses_npf, npf_data.merge({management_unit: nil}))
      undefined_management_unit.save(validate: false)
      ...
      expect(generated.data[:management_unit][undefined_message]).to eq(expected_data[:management_unit][undefined_message])
    end
    ...
    it 'creditor' do
      ...
      undefined_creditor = build(:integration_expenses_npf, npf_data.merge({creditor: nil}))
