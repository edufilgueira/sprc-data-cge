...
describe Integration::Expenses::Neds::CreateSpreadsheet do

  let(:management_unit) { create(:integration_supports_management_unit, poder: 'EXECUTIVO') }

  let(:ned) { create(:integration_expenses_ned, unidade_gestora: management_unit.codigo, exercicio: "#{year}") }

  let(:date) { (Date.today - 2.years).beginning_of_month }
  let(:month) { date.month }
  let(:year) { date.year }

  subject(:service) { Integration::Expenses::Neds::CreateSpreadsheet.new(year, 0) }
  ...
  it 'creates base contract dir' do
    dir = Rails.root + "/public/files/downloads/integration/expenses/neds/#{year}/"
  ...
  it 'overrides existing directory' do
    base_dir = Rails.root + "/public/files/downloads/integration/expenses/neds/#{year}"
  ...
  it 'resources' do
    ...
    expected = Integration::Expenses::Ned.
      from_executivo.
      from_year(year).
      ordinarias
  ...
  context 'spreadsheet' do
    it 'adds worksheet with title' do
      ...
      expected_title = I18n.t("integration/expenses/ned.spreadsheet.worksheets.default.title", year: year).slice(0, 25)
    ...
    it 'adds header for worksheet_type' do
      ned
      expect(service).to receive(:xls_add_header).with(anything, Integration::Expenses::Ned::SpreadsheetPresenter.spreadsheet_header)
  ...
  context 'csv' do
    it 'converts xls to csv' do
      ...
      xls_filename = Rails.root.to_s + "/public/files/downloads/integration/expenses/neds/#{year}/notas_de_empenho_#{year}.xlsx"
      csv_filename = Rails.root.to_s + "/public/files/downloads/integration/expenses/neds/#{year}/notas_de_empenho_#{year}.csv"
