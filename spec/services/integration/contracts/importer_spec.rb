...
#http://sacc.cge.ce.gov.br/UploadArquivos/20171227.1034649.Integra.CONTRATO.pdf

describe Integration::Contracts::Importer do
  let(:contract_parameters) { 'dataAssinaturaInicial=01/01/2017&dataAssinaturaFinal=31/03/2017' }
  let(:additive_parameters) { 'dataAditivoInicial=01/01/2017&dataAditivoFinal=31/03/2017' }
  let(:adjustment_parameters) { 'dataAditivoInicial=01/01/2017&dataAditivoFinal=31/03/2017' }
  let(:financial_parameters) { 'dataDocumentoInicial=01/01/2017&dataDocumentoFinal=31/03/2017' }
  let(:infringement_parameters) { '' }

  let(:configuration) { create(:integration_contracts_configuration, contract_parameters: contract_parameters, additive_parameters: additive_parameters, adjustment_parameters: adjustment_parameters, financial_parameters: financial_parameters, infringement_parameters: infringement_parameters) }
  ...
  let(:service) { Integration::Contracts::Importer.new(configuration.id) }
  ...
  let(:contrato) do
    {
      ...
      :data_auditoria => '20/10/2010',
    }
  end

  let(:contrato_response) do
    {
      consulta_contratos_convenios_response: {
        lista_contratos_convenios: {
  ...
  let(:contrato_message) do
    {
      ...
      data_assinatura_inicial: '01/01/2017',
      data_assinatura_final: '31/03/2017'
    }
  end
  ...
  before do
    ...
    allow(service.client).to receive(:call).
      with(:consulta_contratos_convenios, advanced_typecasting: false, message: contrato_message) { contract_response }
  ...
  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Integration::Contracts::Importer).to receive(:new).with(1) { service }
      allow(service).to receive(:call)
      Integration::Contracts::Importer.call(1)

      expect(Integration::Contracts::Importer).to have_received(:new).with(1)
  ...
  describe 'import_contracts' do
    it 'creates contracts' do
      expect do
        ...
        expect(created_contract.data_auditoria).to eq(Date.parse('20/10/2010'))

      end.to change(Integration::Contracts::Convenant, :count).by(1)
    end

    it 'does not override old data_auditoria' do
      ...
      contrato[:data_auditoria] = 'Sat, 01 Jan 0001'
      service.send(:start)
      service.send(:import_contracts)
    ...
    it 'create contract with confidential' do
      service.send(:start)
      service.send(:import_contracts)

      contrato[:confidential] = true

      service.send(:start)
      service.send(:import_contracts)
      created_contract = Integration::Contracts::Convenant.last

      expect(created_contract.confidential).to eq(true)
  ...
  describe 'import_additives' do
    let(:message) do
      {
        usuario: configuration.user,
        senha: configuration.password,
        data_aditivo_inicial: '01/01/2017',
        data_aditivo_final: '31/03/2017'
      }
    end

    let(:body) do
      {
        consulta_aditivo_response: {
          lista_aditivo: {
            aditivo: [additive]
          }
        }
    ...
    before do
      response = double()
      allow(service.client).to receive(:call).
        with(:consulta_aditivo, advanced_typecasting: false, message: message) { response }
      allow(response).to receive(:body) { body }
      service.send(:start)
    end

    it 'creates additives' do
      expect do
        create(:integration_contracts_contract, isn_sic: "978601")
        service.send(:import_additives)
      end.to change(Integration::Contracts::Additive, :count).by(1)
    end

    it 'creates additives confidential' do
      contrato[:confidential] = true
      ...
      additive = Integration::Contracts::Additive.last

      expect(contract.additives.last).to eq(additive)
      expect(contract.additives.last.descricao_url).to be(nil)      
    end
    ...
    # Comentado porque o teste abaixo já faz esse teste
    # e precisa de um contrato criado pra testar o aditivo
    #
    # it 'creates additives' do
    #   expect do
  ...
  describe 'import_adjustments' do
    ...
    let(:body) do
      {
        consulta_apostilamento_response: {
          lista_apostilamento: {
    ...
    before do
      response = double()
      allow(service.client).to receive(:call).
        with(:consulta_apostilamento, advanced_typecasting: false, message: message) { response }
      allow(response).to receive(:body) { body }
      service.send(:start)
    end

    # Comentado porque o teste abaixo já faz esse teste
    # e precisa de um contrato criado pra testar o adjustment
    #
    # it 'creates adjustments' do
    #   expect do
    #     service.send(:import_adjustments)
    #   end.to change(Integration::Contracts::Adjustment, :count).by(1)
    # end

    it 'creates adjustments confidential' do
      contrato[:confidential] = true
      ...
      contract = Integration::Contracts::Convenant.last
      ajuste = contract.adjustments.last

      expect(ajuste.contract).to eq(contract)
      expect(contract.confidential).to eq(true)
      expect(ajuste.descricao_url).to be(nil)
  ...
  describe 'import_financials' do
    let(:message) do
      {
        usuario: configuration.user,
        senha: configuration.password,
        data_documento_inicial: '01/01/2017',
        data_documento_final: '31/03/2017'
      }
    end

    let(:body) do
      {
        consulta_financeiro_response: {
          lista_financeiro: {
    ...
    before do
      response = double()
      allow(service.client).to receive(:call).
        with(:consulta_financeiro, advanced_typecasting: false, message: message) { response }
      allow(response).to receive(:body) { body }
      service.send(:start)
    end
  ...
  describe 'stats' do
    ...
    describe 'change monitoring' do
      ...
      it 'only create_stats when new data is imported' do
        ...
        expect(Integration::Contracts::Contracts::CreateStats).to receive(:call).with(Date.today.year, Date.today.month)
        expect(Integration::Contracts::Contracts::CreateStats).not_to receive(:call).with(2017, 1)
        ...
        expect(Integration::Contracts::ManagementContracts::CreateStats).to receive(:call).with(Date.today.year, Date.today.month)
        expect(Integration::Contracts::ManagementContracts::CreateStats).not_to receive(:call).with(2017, 1)
        ...
        expect(Integration::Contracts::Convenants::CreateStats).to receive(:call).with(Date.today.year, Date.today.month)
        expect(Integration::Contracts::Convenants::CreateStats).not_to receive(:call).with(2017, 1)
        expect(Integration::Contracts::Convenants::CreateStats).not_to receive(:call).with(2017, 2)
  ...
  describe 'stats' do
    ...
    describe 'change monitoring' do
      ...
      it 'only create_stats when new data is imported' do
        ...
        expect(Integration::Contracts::Contracts::CreateSpreadsheet).to receive(:call).with(Date.today.year, Date.today.month)
        expect(Integration::Contracts::Contracts::CreateSpreadsheet).not_to receive(:call).with(2017, 1)
        ...
        expect(Integration::Contracts::ManagementContracts::CreateSpreadsheet).to receive(:call).with(Date.today.year, Date.today.month)
        expect(Integration::Contracts::ManagementContracts::CreateSpreadsheet).not_to receive(:call).with(2017, 1)
        ...
        expect(Integration::Contracts::Convenants::CreateSpreadsheet).to receive(:call).with(Date.today.year, Date.today.month)
        expect(Integration::Contracts::Convenants::CreateSpreadsheet).not_to receive(:call).with(2017, 1)
        expect(Integration::Contracts::Convenants::CreateSpreadsheet).not_to receive(:call).with(2017, 2)
