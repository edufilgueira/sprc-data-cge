...
describe Integration::Contracts::Contracts::CreateSpreadsheet do

  let(:contract) { create(:integration_contracts_contract, :contrato, data_assinatura: date - 1.year, data_termino: date + 1.year) }
  let(:convenant) { create(:integration_contracts_contract, :convenio, data_assinatura: date - 1.year, data_termino: date + 1.year) }
  let(:date) { (Date.today - 2.years).beginning_of_month }
  let(:month) { date.month }
  let(:year) { date.year }
  let(:year_month) { I18n.l(Date.new(year, month), format: '%Y%m') }

  subject(:service) { Integration::Contracts::Contracts::CreateSpreadsheet.new(year, month) }
  ...
  it 'creates base contract dir' do
    dir = Rails.root + "/public/files/downloads/integration/contracts/contracts/#{year_month}/"
  ...
  it 'overrides existing directory' do
    base_dir = Rails.root + "/public/files/downloads/integration/contracts/contracts/#{year_month}"
  ...
  context 'csv' do
    it 'converts xls to csv' do
      ...
      xls_filename = Rails.root.to_s + "/public/files/downloads/integration/contracts/contracts/#{year_month}/contratos_#{year_month}.xlsx"
      csv_filename = Rails.root.to_s + "/public/files/downloads/integration/contracts/contracts/#{year_month}/contratos_#{year_month}.csv"
