...
describe Integration::Revenues::Configuration do

  subject(:configuration) { build(:integration_revenues_configuration) }

  describe 'factories' do
    it { is_expected.to be_valid }

    it { expect(build(:integration_revenues_configuration, :invalid)).to be_invalid }
  end
  ...
  describe 'associations' do
    it { is_expected.to have_many(:account_configurations) }
    it { is_expected.to have_one(:schedule) }
  end

  describe 'nested' do
    it { is_expected.to accept_nested_attributes_for(:account_configurations) }
  ...
  describe 'helpers' do
    ...
    it 'import' do
      ...
      expect(Integration::Revenues::Importer).to receive(:delay) { importer }
