...
describe Integration::Results::Configuration do
  subject(:configuration) { build(:integration_results_configuration) }

  describe 'factories' do
    it { is_expected.to be_valid }

    it { expect(build(:integration_results_configuration, :invalid)).to be_invalid }
  end
  ...
  describe 'db' do
    describe 'columns' do
      ...
      it { is_expected.to have_db_column(:strategic_indicator_operation).of_type(:string) }
      it { is_expected.to have_db_column(:thematic_indicator_operation).of_type(:string) }
      it { is_expected.to have_db_column(:strategic_indicator_response_path).of_type(:string) }
      it { is_expected.to have_db_column(:thematic_indicator_response_path).of_type(:string) }
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_presence_of(:strategic_indicator_operation) }
    it { is_expected.to validate_presence_of(:strategic_indicator_response_path) }
    it { is_expected.to validate_presence_of(:thematic_indicator_operation) }
    it { is_expected.to validate_presence_of(:thematic_indicator_response_path) }
  ...
  describe 'helpers' do
    ...
    it 'import' do
      ...
      expect(Integration::Results::Importer).to receive(:delay) { importer }
