...
describe Integration::Results::ThematicIndicator do
  subject(:strategic_indicator) { build(:integration_results_thematic_indicator) }

  describe 'factories' do
    it { is_expected.to be_valid }

    it { expect(build(:integration_results_thematic_indicator, :invalid)).to be_invalid }
  end

  describe 'db' do
    describe 'columns' do
      it { is_expected.to have_db_column(:eixo).of_type(:jsonb) }
      it { is_expected.to have_db_column(:tema).of_type(:jsonb) }
      ...
      it { is_expected.to have_db_column(:valores_programados).of_type(:jsonb) }
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_presence_of(:tema) }
  ...
  describe 'delegations' do
    ...
    it { is_expected.to delegate_method(:title).to(:theme).with_prefix }
  end
end
